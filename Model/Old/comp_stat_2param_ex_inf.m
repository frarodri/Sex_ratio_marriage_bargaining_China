%**************************************************************************
% Comparative statics - two parameters and exogenous inflow of singles    *
...
param.theta = 1.2; % Sex ratio among new entrants
...
theta_S_vec = zeros(size(values_first,2),size(values_second,2));
P_f_S_vec = zeros(size(param.P_f,1),size(values_first,2),...
    size(values_second,2));
P_m_S_vec = zeros(size(param.P_m,1),size(values_first,2),...
    size(values_second,2));
...
for i = 1:size(values_first,2)
    ...
    for j = 1:size(values_second,2)
       ...
       elseif type_second == 3
        ...
        % Compute actual stationary sex ratio and distribution of types 
        % over singles
        [theta_S, P_f, P_m] = singles_dist(Q,param);
        theta_S_vec(i,j) = theta_S;
        P_f_S_vec(:,i,j) = P_f;
        P_m_S_vec(:,i,j) = P_m;
        ...
        % Marriage rates
        MR_f(:,i,j) = pi_f*(1-normcdf(Q,param.MU))*P_m;
        MR_m(:,i,j) = pi_m*(1-normcdf(Q,param.MU))'*P_f;

        mr_f(i,j) = P_f'*MR_f(:,i,j);
        mr_m(i,j) = P_m'*MR_m(:,i,j);
